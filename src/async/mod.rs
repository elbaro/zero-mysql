...
/// Iterator over query results that reads and decodes rows on demand (async)
...
impl<'a> QueryResult<'a> {
    fn new(reader: &'a mut BufReader<TcpStream>, num_columns: usize, columns: Vec<ColumnDefinition>) -> Self {
        Self {
            reader,
            buffer: Vec::new(),
            row_payload: Vec::new(),
    ...
    /// Read the next row packet asynchronously
    ///
    /// Returns `Ok(Some(Row))` if a row was read, `Ok(None)` if no more rows, `Err` on error
    pub async fn next_row(&mut self) -> Result<Option<Row<'_>>> {
        if self.finished {
            return Ok(None);
        }

        let _seq = read_payload(self.reader, &mut self.buffer).await?;
