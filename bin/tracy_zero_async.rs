...
async fn main() -> Result<()> {
    ...
    let mut conn = Conn::new("mysql://test:1234@localhost/test").await?;
    ...
    // Drop and recreate the test table using MEMORY engine
    {
        ...
        conn.exec(drop_stmt, &empty_params, &mut decoder).await?;
        ...
        let mut decoder = Handler::new();
        conn.exec(create_stmt, &empty_params, &mut decoder).await?;
    }
    ...
    loop {
        ...
        // Insert 10,000 rows
        {
            ...
            for (username, age, email, score, description) in &rows {
                ...
                );
                {
                    ...
                    conn.exec(insert_stmt, &insert_params, &mut insert_decoder).await?;
                }
            }
        }
        ...
        // Truncate the table
        {
            ...
            let empty_params: [i32; 0] = [];
            conn.exec(truncate_stmt, &empty_params, &mut truncate_decoder).await?;
        }
